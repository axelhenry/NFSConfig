...
2. Télécharger une image .iso d'une distribution Linux (par habitude, la distribution [Archlinux](https://www.archlinux.org/download/) sera utilisée ici, mais beaucoup d'autres peuvent faire l'affaire).

3. Créer une nouvelle machine virtuelle (noyau 3.x, 32 bits, 128mo de ram et 2go d'espace disque devraient être suffisants.).

1. Passer l'adaptateur réseau de votre machine virtuelle en mode Bridged.

4. Installer votre distribution (se référer aux tutos de votre distribution), pour économiser de la place vous pouvez vous dispenser d'installer une interface graphique.
...
6. Eteindre votre machine virtuelle, activer les dossiers partagés et ajouter les dossiers que vous voulez mettre à disposition de votre machine virtuelle.

7. Prendre une tasse de café ou autre, parce que les choses vont se corser.

8. Redémarrer votre machine virtuelle.

9. Créer les points de montage de vos dossiers partagés (mes dossiers partagés sont ici __HD__, __Series__ et __lowres__).
```sh
sudo mkdir /mnt/hd
...
10. Ouvrir le fichier /etc/fstab
```sh
sudo nano /etc/fstab
```
...
12. Redémarrer votre machine virtuelle, et exécuter
```sh
ls /mnt/hd
```
Vous devriez voir le contenu de votre dossier partagé.

13. Créer les points de montage nfs.
```sh
...
14. Ouvrir le fichier /etc/fstab
```sh
sudo nano /etc/fstab
```
et ajouter ces lignes :
...
16. Ouvrir le fichier /etc/exports
```sh
...
2. Créer les points de montage de nos partages.
```sh
sudo mkdir /media/hd
...
```sh
sudo wget -p /usr/local/bin http://raw.github.com/axelhenry/NFSConfig/files/auto_share.sh
```
...
```sh
sudo wget -p /etc/systemd/system/ http://raw.github.com/axelhenry/NFSConfig/files/auto_share.timer
```

1. Télécharger le service auto_share.service :
```sh
sudo wget -p /etc/systemd/system/ http://raw.github.com/axelhenry/NFSConfig/files/auto_share.service
```
